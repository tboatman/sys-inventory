...
FIXTURE_TO_REAL_NAME = {
    ...
    "sample_opt_snapshot.txt": "opt_snapshot.txt",
    "sample_clock_snapshot.txt": "clock_snapshot.txt",
    "sample_autor_snapshot.txt": "autor_snapshot.txt",
    ...
    "sample_igdsms_snapshot.txt": "igdsms_snapshot.txt",
    "sample_izuprm_snapshot.txt": "izuprm_snapshot.txt",
    "sample_diag_snapshot.txt": "diag_snapshot.txt",
}
...
def test_ingest_populates_every_domain(full_input_dir, tmp_path):
    ...
    try:
        ...
        assert len(store.all_opt_statements(conn)) > 0
        assert len(store.all_clock_statements(conn)) > 0
        assert len(store.all_autor_statements(conn)) > 0
        ...
        assert len(store.all_igdsms_statements(conn)) > 0
        assert len(store.all_izuprm_statements(conn)) > 0
        assert len(store.all_diag_statements(conn)) > 0
...
NO_ARG_SUBCOMMANDS = [
    ...
    "ieasys", "bpxprm", "devsup", "opt", "clock", "autor", "sched", "couple", "grscnf", "grsrnl",
    "smf", "ios", "consol", "igdsms", "izuprm", "diag", "iggcat", "prog", "ieasvc", "lpalst", "mlpa", "fix", "vatlst", "active", "processes", "catalog", "vsam",

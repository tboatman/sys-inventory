...
EXPECTED_TABLES = {
    ...
    "mlpa_statements", "fix_statements", "vatlst_defaults", "vatlst_entries",
...
ROUND_TRIP_CASES = [
    ...
    pytest.param(
        store.save_couple_statements, store.all_couple_statements,
        lambda n: [CoupleStatement(stmt=f"STMT{i}", operands="OP", source_member="COUPLE00") for i in range(n)],
        id="couple_statements",
    ),
    pytest.param(
        store.save_grsrnl_statements, store.all_grsrnl_statements,
        lambda n: [GrsrnlStatement(stmt="RNLDEF", operands=f"QNAME(Q{i})", source_member="GRSRNL00") for i in range(n)],
        id="grsrnl_statements",
    ),
    pytest.param(
        store.save_smf_statements, store.all_smf_statements,
        lambda n: [SmfStatement(stmt=f"STMT{i}", operands="OP", source_member="SMFPRM00") for i in range(n)],
        id="smf_statements",
    ),
    pytest.param(
    ...
    pytest.param(
        store.save_mlpa_statements, store.all_mlpa_statements,
        lambda n: [MlpaStatement(stmt="INCLUDE", operands=f"LIBRARY(SYS1.LPALIB{i})", source_member="IEALPA00") for i in range(n)],
        id="mlpa_statements",
    ),
    pytest.param(
        store.save_fix_statements, store.all_fix_statements,
        lambda n: [FixStatement(stmt="INCLUDE", operands=f"LIBRARY(SYS1.LPALIB{i})", source_member="IEAFIX00") for i in range(n)],
        id="fix_statements",
    ),
    pytest.param(
        store.save_vatlst_defaults, store.all_vatlst_defaults,
        lambda n: [VatlstDefaults(ipluse="(PRIVATE)", sysuse="(PRIVATE)", source_member=f"VATLST{i:02d}") for i in range(n)],
        id="vatlst_defaults",
    ),
    pytest.param(
        store.save_vatlst_entries, store.all_vatlst_entries,
        lambda n: [VatlstEntry(volser=f"VOL{i}", attribute="0", percent_full="0", device_type="3390", convertible="Y", source_member="VATLST00") for i in range(n)],
        id="vatlst_entries",
    ),
    pytest.param(

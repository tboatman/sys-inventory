...
def test_nested_paren_value_with_interleaved_comments_captured_whole():
    ...
    assert ckptdef.params["CKPT1"] == "(DSNAME=SYS1.SYSB.HASPCKPT, VOLSER=SYSBW1, INUSE=YES)"
...
def test_include_statement_captured_generically():
    ...
    assert include.params == {"DSNAME": "SYS1.SYSB.PARMLIB(JES2NJE)", "VOLSER": "SYSBW1"}
...
def test_second_member_parsed_with_its_own_source_member():
    ...
    assert socket.params == {"IPADDR": "SYSP.EXAMPLE.COM", "PORT": "175", "NODE": "1"}

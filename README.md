...
and IEASYSxx/BPXPRMxx/DEVSUPxx member content actually in effect (not just where
...
1. **`zos-extract/`** runs on z/OS, in an OMVS (UNIX) shell, using ZOAU (Z
   ...
   PARMLIB/IEASYSxx/BPXPRMxx/DEVSUPxx snapshots, multi-CSI SMP/E zone discovery, USS
...
`cics-sit`, `cics-csd`, `bpxprm`, `devsup`, `zone-index`, `zones`, `fmids`,
...
PARMLIB/IEASYSxx/BPXPRMxx/DEVSUPxx snapshots, SMP/E's own authoritative per-CSI

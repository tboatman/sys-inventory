...
| Active BPXPRMxx member snapshot (z/OS UNIX/OMVS config, named by IEASYSxx's own OMVS=) | `bpxprm_snapshot` (default filename `bpxprm_snapshot.txt`, override via `zos_extract_bpxprm_snapshot_outfile`) | ansible-only, `bpxprm_snapshot.yml` -- CONFIRMED against a real member |
| Active DEVSUPxx member snapshot (device support definitions, named by IEASYSxx's own DEVSUP=) | `devsup_snapshot` (default filename `devsup_snapshot.txt`, override via `zos_extract_devsup_snapshot_outfile`) | ansible-only, `devsup_snapshot.yml` -- CONFIRMED against a real member |
| Active IEAOPTxx member snapshot (system tuning/options parameters, named by IEASYSxx's own OPT=) | `opt_snapshot` (default filename `opt_snapshot.txt`, override via `zos_extract_opt_snapshot_outfile`) | ansible-only, `opt_snapshot.yml` -- CONFIRMED against a real member |
| Active CLOCKxx member snapshot (TOD clock/timezone parameters, named by IEASYSxx's own CLOCK=) | `clock_snapshot` (default filename `clock_snapshot.txt`, override via `zos_extract_clock_snapshot_outfile`) | ansible-only, `clock_snapshot.yml` -- CONFIRMED against a real member (space-separated shape, see `clock_parser.py`) |
| Active AUTORxx member snapshot (WTOR auto-reply policy, named by IEASYSxx's own AUTOR=) | `autor_snapshot` (default filename `autor_snapshot.txt`, override via `zos_extract_autor_snapshot_outfile`) | ansible-only, `autor_snapshot.yml` -- CONFIRMED against a real member |
| Active SCHEDxx member snapshot (PPT/Program Properties Table entries, named by IEASYSxx's own SCH=) | `sched_snapshot` (default filename `sched_snapshot.txt`, override via `zos_extract_sched_snapshot_outfile`) | ansible-only, `sched_snapshot.yml` -- CONFIRMED against a real member |
| Active COUPLExx member snapshot (XCF/sysplex couple dataset definitions, named by IEASYSxx's own COUPLE=) | `couple_snapshot` (default filename `couple_snapshot.txt`, override via `zos_extract_couple_snapshot_outfile`) | ansible-only, `couple_snapshot.yml` -- CONFIRMED against a real member |
| Active GRSCNFxx member snapshot (Global Resource Serialization configuration parameters, named by IEASYSxx's own GRSCNF=) | `grscnf_snapshot` (default filename `grscnf_snapshot.txt`, override via `zos_extract_grscnf_snapshot_outfile`) | ansible-only, `grscnf_snapshot.yml` -- CONFIRMED against a real member |
| Active GRSRNLxx member snapshot (global resource serialization resource name lists, named by IEASYSxx's own GRSRNL=) | `grsrnl_snapshot` (default filename `grsrnl_snapshot.txt`, override via `zos_extract_grsrnl_snapshot_outfile`) | ansible-only, `grsrnl_snapshot.yml` -- CONFIRMED against a real (partial) member |
...
| Active IEALPAxx member snapshot (Modified Link Pack Area module additions, named by IEASYSxx's own MLPA=) | `mlpa_snapshot` (default filename `mlpa_snapshot.txt`, override via `zos_extract_mlpa_snapshot_outfile`) | ansible-only, `mlpa_snapshot.yml` -- CONFIRMED against a real member |
| Active IEAFIXxx member snapshot (IBM fix/PTF-supplied Link Pack Area module additions, named by IEASYSxx's own FIX=) | `fix_snapshot` (default filename `fix_snapshot.txt`, override via `zos_extract_fix_snapshot_outfile`) | ansible-only, `fix_snapshot.yml` -- CONFIRMED against a real member |
| Active VATLSTxx member snapshot (volume attribute list, named by IEASYSxx's own VAL=) | `vatlst_snapshot` (default filename `vatlst_snapshot.txt`, override via `zos_extract_vatlst_snapshot_outfile`) | ansible-only, `vatlst_snapshot.yml` -- CONFIRMED against a real member |
